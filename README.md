...
No commands needed. No manual copy-pasting. It just works in the background while you stream across **Twitch**, **Kick**, and **YouTube** simultaneously. Works seamlessly in multi-streaming environments. Trovo and TikTok support coming soon.

> ⚠️ **This project is currently in Beta.** Core functionality is stable but features are actively being developed. Feedback welcome via the [Casterlabs Discord](https://casterlabs.co/discord) or [chatincluded.live](https://chatincluded.live).
...
3. Post the translated message back to chat: `[ES->EN] Hello, how are you?`

When the streamer @mentions a viewer whose language has been detected, ChatIncluded automatically translates the reply back into that viewer's language — enabling genuine two-way conversation across language barriers.
...
| Setting | Description |
|---------|-------------|
| !chatincluded response message | The message posted when viewers type !chatincluded. Customizable per streamer |
...
| `!speak <message>` | Configurable | Translates your message into every language active in the session via !setlang |
...
1. A viewer sends a message in Spanish: `hola como estas`
2. ChatIncluded translates and posts: `[ES->EN] hello how are you`
3. The streamer @mentions the viewer in their reply: `@username Thanks for watching!`
4. ChatIncluded detects the @mention, looks up the viewer's language, and posts: `@username [EN->ES] ¡Gracias por ver!`

Viewers can also pin their language with `!setlang ES` to ensure accurate translations even for short messages that might be misdetected.
...
Make sure you created a ChatIncluded Settings widget instance and entered your API key. Check that your API Plan matches your actual DeepL account type.
...
**Bot messages are being translated**
Add the bot's username to the Excluded Usernames field in the Bot Exclusions settings tab (comma-separated, case-insensitive).
...
**Two-way @mention reply isn't triggering**
The viewer must have sent at least one message that ChatIncluded translated within the last 30 minutes (configurable). Use `!setlang` to permanently register a viewer's language for the session.
...
- [x] Chat commands (!chatincluded, !translate, !setlang, !languages, !speak)
- [x] Bot exclusion list
- [ ] Trovo support
- [ ] TikTok support
- [ ] chatincluded.live website with full documentation
